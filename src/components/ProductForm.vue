<template>
  <form id="product-form" @submit="saveProduct">
    <div class="form-control">
      ...
      <input type="text" id="title" v-model.trim="inputState.title" />
    </div>
    <div class="form-control">
      <label for="price">Price</label>
      <input type="number" min="0" id="price" step="0.01" v-model.trim="inputState.price" />
    </div>
...
export default {
 ...
 const isValid = computed(() => {
      ...
      if (inputState.title.length === 0 ||
        isNaN(priceAsNumber.value) ||
        priceAsNumber.value <= 0) {
        isValid = false;
      }
      return isValid;
    });

 // watch([inputState,priceAsNumber],(newArr,oldArr)=>{
 //    let isValid=true;
 //    if (newArr[0].title.length === 0 ||
 //        isNaN(newArr[1].value) ||
 //        newArr[1].value <= 0) {
 //        isValid = false;
 //      }
 // })
